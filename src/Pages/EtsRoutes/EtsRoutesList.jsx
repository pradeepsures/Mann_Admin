...
export default function EtsRouteList() {
  ...
  const { hasPermission } = useAuth();
    ...
    const fetchRoutes = useCallback(async () => {
        try {
            ...
            const res = await getAllEtsRoutes({
                page,
                rowsPerPage,
                ...filters,
            });
            ...
            if (res?.status) {
                const formatted = res.data.map((item) => ({
                    ...item,
                    id: item._id,
                    stoppageCount: item?.stoppages?.length || 0,
                }));
    ...
    const deleteHandler = (id) => {
        Modal.confirm({
            ...
            okType: "danger",
            onOk: async () => {
                const res = await deleteEtsRouteApi(id);
                if (res?.status) {
    ...
    const handleToggle = async (id) => {
        try {
            setToggleLoading(id);
            const res = await toggleEtsRouteStatusApi(id);
            if (res?.status) {
    ...
    const exportExcel = () => {
        if (data.length < 1) return toast.error("No data to export");
        ...
        const exportData = [
            {
                sheet: "ETS Routes",
                columns: [
                    { label: "Route Name", value: "name" },
                    { label: "Total Stoppages", value: "stoppageCount" },
                    {
                        label: "Status",
                    ...
                    //OPTIONAL (if you want stoppages detail in one column)
                    {
                        label: "Stoppages",
                        value: (r) =>
                            r.stoppages?.map((s) => s.name).join(" | ") || "-",
                    },
                ],
                content: data,
            },
        ...
        xlsx(exportData, { fileName: "ETS_Route_List" });
    ...
    {hasPermission("EtsUsers", "create") && (
                <motion.button
                    onClick={() => navigate("/home/etsRoutes/create")}
                    className="bg-primary text-white px-5 py-2 rounded-lg"
                >
                    Add Route
                </motion.button>
                                    ...
                                    >
                                     ...
                                     <Menu
                                                        ...
                                                        >
                                                          ...
                                                          {hasPermission("EtsRoutes", "update") && (
                                                            <MenuItem onClick={() => navigate(`update/${row.id}`)}>
                                                          ...
                                                          {hasPermission("EtsRoutes", "delete") && (
                                                            <MenuItem onClick={() => deleteHandler(row.id)}>
